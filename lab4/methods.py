...
print(f"Точне аналітичне значення: {exact_val:.6f}\n")

# Задаємо фіксований крок за методичкою
h = 1e-3

# Базові розрахунки для трьох різних кроків (h, 2h, 4h)
D_h = num_dM(t0, h)
D_2h = num_dM(t0, 2 * h)
D_4h = num_dM(t0, 4 * h)
R1 = np.abs(D_h - exact_val)
print("1. Звичайне чисельне диференціювання (h = 0.001):")
print(f"   Значення: {D_h:.6f}")
print(f"   Похибка R1: {R1:.2e}\n")
# Метод Рунге-Ромберга (використовує h та 2h)
D_R = D_h + (D_h - D_2h) / 3
...
# Метод Ейткена (використовує h, 2h та 4h)
...
print(f"   (Похибка зменшилась у {R1/R3:.1f} разів)")